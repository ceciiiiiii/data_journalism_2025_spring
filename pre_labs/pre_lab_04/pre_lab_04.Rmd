...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer: There are several grantees who spelled their names slightly differently each time they got a grant. So it's difficult to see how many recipients there actually are.  **
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer: There are several instances where grantee_clean capitalized the entire name. So Mount St Marys University is written as MOUNT ST MARYS UNIVERSITY. Another example is it added LLC at the end of Rise Therapeautics. So it found the full name for Rise Therapeutics LLC.**
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer:I could only find two changes within the first 30 results. The first change I see is it removed the Inc. at the end of Walter P Carter Child Care Center. The second is it changed Washington Adventist University into all caps. The third change is it consolidated all 13 instances of Ward Foundation Inc. into one in grantee_clean. **
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer: There is no substantial change from the previous use of key_collision_merge. From what I can see in the first 30 rows there's no difference between the first cleaning function and the second.  **
