...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer: It no longer shows any parsing errors. **
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer: The Vendor Name field is a character data type, so it has the names of the vendors. The Amount data field is a double data type, so it has the amount of money paid. The date column is a character column, but this can be changed to a double data type.**
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think explains those records? **Answer: 553 zip codes are less than five characters in length. It's probably user error. A number may have been forgotten or cut off. Zip codes can also include an additional four digits which in tandem with the original five digit code gives even more geographic information. There may have been some mix-ups there.  **
...
**Task** Run the following codeblock and describe below what each line is doing. Looking at the results, what jumps out at you as potentially problematic? **Answer:There was obvious inconsistency with the way each row was categorized. Some of them are N/A, there's one labeled as contract, others are L instead of what I assume to be Loan. Also, there's no data for Loans in 2009. **
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer: The smallest amount given within the time period of this data set was a grant of $60 in 2009 by the Department of Natural Resources/Rural Legacy Program. When I run the code it only shows up to 1000 of the 19,000 rows. So when I went and checked the database the largest amount ever given was 767,888,790 in 2020 by the Department of Education for teacher retirement funds.**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does it change the headers? Answer below. **Answer: The clean_names function makes all the header names lowercase and uses underscores instead of spaces in the header names. **
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. Describe what each line is doing. **Answer: The first line creates a new datavset using the data from md_grants_loans. Then the next line clean_names standardizes the way the headers are written. The next line changes the name of the column from grantor to source. The next two lines makes all the data in source, grantee and description uppercase. The final line displays the new data set. **
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer: There are 58 possible duplicates. **
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer: The new dataset has 19,453 rows. **
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer: A new column with the header zip5 was added at the end which only has the first five digits of every zipcode. **
...
**Task** Write and run code to answer our original question: which zip code has gotten the most amount of money from the Maryland Tourism Board? Where is that zip code, and does that zip code make sense to you as the top recipient? Write a sentence describing the top result. **Answer: The zip code which has received the most amount of money from the Maryland Tourism Board is 21202. Which is the zip code for part of the inner harbor/downtown area of Baltimore City. Since it is on the water I'm assuming a lot of maintenance has to be done on the docks to ensure their safety. There has also been huge efforts to renovate this area in recent years. So it doesn't surprise me they were the top recipient. The Baltimore Inner Harbor has received more than $600,000 from the Maryland Tourism Board between 2009 and 2023. More than any other zipcode in Maryland. **
