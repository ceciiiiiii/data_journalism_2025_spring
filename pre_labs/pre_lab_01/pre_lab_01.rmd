...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer: The id column is text, specifically the class numbers and the seats is the number of students who can take a class.**
...
In the space below, write the name of the business from the first row of the "name" column. **Answer: I'm not sure if it's referring to the title column instead which would be Directed Research. **
...
**Answer: The first line groups the data from each semester/term and then the second line looks at each group and calculates how many classes are in each term.  **
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: Now the data shows up in the order of the  terms with the most amount of classes to the least amount of classes.**
...
**Answer: Now the code is outputting the number of classes each department has in a term. The code ties together two variables in a row and then calculates the number for those two variables together.  **
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: Now the code is calculating two totals for each term. The numbers for terms and classes is the same but now there's an extra piece of information linked to it. **
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer: The new codeblock adds on two functions to calculate the mean number of seats in a class in a term and then the median number of seats per class in a term. The top terms are all fall terms. **
...
**Answer: The data is now grouped by department so instead of focusing on one term it looks at overal class seats in each different department. **
...
1.  Is the biggest school on campus the one with the most available seats?

2.  Is there a correlation between the mean number of seats and grades

3.  Why do some classes have more than 1000 seats available?
