...
**Task** Run the following code to load the data. In the space below, indicate how many rows and columns it has. **Answer: There are 3822 rows and 4 columns.**
...
**Task** Run the following code to add a month column to our dataframe using lubridate and count the number of calls for each month. In the space below, explain what each step of the code below is doing in English. Note the "label=TRUE" part in the mutate statement. **Answer: We've added the data from all Baltimore 911 calls into a new data base. Next we're changing the dates from individual dates to months. Next we're making a column where we're counting up all the total calls in each month and then arranging from the lowest to the highest number of columns**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: There's just a blank white square. This is where I assume the chart will go.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: There's a bar chart with a count number on the y axis and the months along the x axis. Each column/bar symbolizes the number of calls made in that month. **
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer: Now the columns are arranged from lowest to highest number of calls made in that month. The codeblock now features a reorder function which changes the order of the months based on the actual content in that column instead of by month order.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer: This code now features the function coord_flip() which basically just switched the axises. Now the bars run horizontally instead of vertically.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer: The background is no longer light gray but instead just plain white. By adding a theme function (theme_minimal()) the colors changed.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer: Now the background is a light blue and there is no grid background.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer: The chart now has a title and the y axis is now correctly labeled. The code now has the function labs where the title and axises were labeled accordingly. A caption was also added.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: There is now a data base where one column has the date and the other the total calls made on that day.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: There is a line chart with total calls on the y axis and dates along the x axis. The chart is virtually unreadable due to so many data points being crowded together.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: there is a tick marck for every date on the bottom axis but since there are so many dates it is to jumbled to differentiate.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer: Now the dates are show at a 45 degree angle so while they are still difficult to read you can make out individual dates.**
